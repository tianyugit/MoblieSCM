...
<html>
  ...
  <body>
    ...
    <form action="pay_editPay.do" method="post" >
      <table width="500" align="center" style="margin-top:30px" class="inputTable">    
        
        <tr>
          <th>供应商：</th>
          <td>
          <input type="hidden" name="payment.invoiceid" value="${payment.invoiceid}">
            <select name="payment.supplierid" class="text">
                <c:forEach items="${supList}" var="sup">
                    <option value="${sup.supid}"
                     <c:if test="${sup.supid eq payment.supplierid}">selected</c:if>>${sup.supname}
                    </option>
                </c:forEach>
            </select>
          </td>
        ...
        <tr>
          <th>金额：</th>
          <td><input class="text" name="payment.paymoney" value="${payment.paymoney}"/></td>
        </tr>    
        <!-- <tr>
            <th>是否计算现金返利</th>
            <td>
        ...
        <tr>
          <th>备注：</th>
          <td><textarea class="text" name="payment.remark" style="width:300px;height:46px">${payment.remark}</textarea></td>
        </tr>       
      </table>
