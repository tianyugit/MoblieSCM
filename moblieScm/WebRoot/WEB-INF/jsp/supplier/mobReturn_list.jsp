...
<html>
  ...
  <body>
    ...
    <table class="table" rowNum="15" width="90%" align="center" >
      ...
      <c:forEach items="${mobreturnList}" var="mr">
        <tr>
          <td>${mr.invoiceid}</td>
          <td><%-- ${mr.supName } --%>
          <c:forEach items="${supList}" var="sup">
          ...
          <td>${mr.remark}</td>
          <td>          
              <a href="mobReturn_showDetail.do?mobreturn.invoiceid=${mr.invoiceid}">详情</a>
              <c:choose>
                  <c:when test="${mr.status eq 0}">
                      <a href="#">修改</a>              
                      <a href="mobReturn_delete.do?mobreturn.invoiceid=${mr.invoiceid}">删除</a>
                      <c:if test="${loginUser.role == 'admin'}">
                        <a href="mobReturn_check.do?mobreturn.invoiceid=${mr.invoiceid}">审核</a>
                      </c:if>
                  </c:when>
                  <c:otherwise>
