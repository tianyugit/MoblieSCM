...
<html>
  <head>
	...
	<body>
		...
		<form action="mobStockIn_add.do" method="post" name="myform" onsubmit="getData()">
			<table class="inputTable" width="90%" align="center" style="margin-top:10px">
					...
					<td>
						<select name="msi.config" class="text" style="width:100">
							<c:forEach items="${configList}" var="con">
								<option value="${con.config}">${con.config}</option>
							</c:forEach>
						</select>
					</td>
					<th>进价(单价)</th>
				...
				<tr>
					...
					<td colspan="7">
						<textarea rows="2" cols="82" name="msi.remark"></textarea>
					</td>
				</tr>
			</table>
