...
<html>
  ...
  <body>
    ...
    <form action="mobType_findMobtypeBySelected.do" method="post">
            ...
            <select class="text" name="mobType.brand">
                ...
                <c:forEach items="${brandList}" var="brand">
                    <option value="${brand.brand}"
                        <c:if test="${mobType.brand eq brand.brand}">selected</c:if>
                    >${brand.brand}</option>
                </c:forEach>
            </select>
          </td>
   ...
   <table class="table" rowNum="15" width="90%" align="center" >
      ...
      <c:forEach items="${mobTypeList}" var="mtype">
        <tr>
          ...
          <td>${mtype.remark}</td>
          <td>          
              <a href="mobType_toEdit.do?mobType.mobtypeid=${mtype.mobtypeid}">编辑</a>
              <a href="mobType_delType.do?mobType.mobtypeid=${mtype.mobtypeid}" onclick="return confirm('确定要删除吗？');">删除</a>
          </td>
        </tr>
      </c:forEach>
    </table>
