...
<html>
  ...
  <body>
    ...
    <form action="accessoryType_findAccessoryType.do" method="post">
            ...
            <select class="text" name="accessoryType.brand">
                ...
                <c:forEach items="${brandList}" var="brand">
                    <option value="${brand.brand}"
                        <c:if test="${accessoryType.brand eq brand.brand}">selected</c:if>
                    >${brand.brand}</option>
                </c:forEach>
            </select>
          </td>
    ...
    <table class="table" rowNum="15" width="90%" align="center" >
      ...
      <c:forEach items="${accessorytypeList}" var="atype">
        <tr>
          ...
          <td>${atype.remark}</td>
          <td>          
              <a href="accessoryType_toEdit.do?accessoryType.accessorytypeid=${atype.accessorytypeid}">编辑</a>
              <a href="accessoryType_delType.do?accessoryType.accessorytypeid=${atype.accessorytypeid}" onclick="return confirm('确定要删除吗？');">删除</a>
          </td>
        </tr>
      </c:forEach>
    </table>
