  ...
  <body>
    ...
    <form action="accessoryType_addType.do" method="post" onsubmit="return ValidateFrm.submitValidate(this)">
      <table width="500" align="center" style="margin-top:30px" class="inputTable">    
        <tr>
          <th>类别编号：</th>
          <td><input class="text" name="accessoryType.accessorytypeid"  value="${accessoryType.accessorytypeid}"/></td>
        </tr>      
        <tr>
          <th>类别名称：</th>
            ...
            <select name="accessoryType.brand" class="text">
                <c:forEach items="${brandList}" var="brand">
                    <option value="${brand.brand}"
                        <c:if test="${brand.brand eq accessoryType.brand}">selected</c:if>
                    >${brand.brand}</option>
                </c:forEach>
            </select>
          </td>
      ...
      <table border="0" width="500" align="center" style="margin-top:20px">
        <tr><td align="center">
          ...
          <input type="button" class="btn cancel" value="取消" onclick="location='accessoryType_list.do'"/>
        </td></tr>
      </table>
    </form>    
  </body>
