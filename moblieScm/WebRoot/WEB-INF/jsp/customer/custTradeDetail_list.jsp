...
<html>
  ...
  <body>
    ...
    <form action="custTradeDetail_list.do" method="post">
    ...
    <table class="table" rowNum="15" width="90%" align="center" >
      ...
      <c:forEach items="${custTradedetailList}" var="msi">
        <tr>
          <td>${msi.invoiceid}</td>
          <%-- <td>
          <c:forEach items="${supList}" var="sup">
         ...
         <%--  <td>${msi.type}</td> --%>                   
          <td>
           <c:if test="${msi.tradetype==1 }">销出</c:if>
          <c:if test="${msi.tradetype==2 }">退货</c:if>
          </td>
          <td>${msi.unitprice}</td>
          <td>${msi.amount}</td>
          <td>
          ...
          <td>${msi.remark}</td>
          <td>          
              <a href="custTradeDetail_delete.do?invoiceid=${msi.invoiceid}">删除</a>
             <%-- <c:choose>
